...
function TabButton({ active, onClick, icon: Icon, children }) {
  return (
    <button onClick={onClick} className={["inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm transition cursor-pointer",
      active ? "border-slate-200 bg-white text-slate-900 shadow-sm" : "border-transparent bg-slate-50 text-slate-600 hover:bg-slate-100"].join(" ")}>
...
export default function VoyDetailPage() {
  ...
  const {
    ...
    fetchAll, fetchActivities, fetchFuel,
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await fetchAll(voyageId); }
      catch (e) { await alertError("โหลดข้อมูลไม่สำเร็จ", e?.response?.data?.message || e?.message || "ไม่สามารถโหลดข้อมูลได้"); }
    })();
  }, [voyageId, fetchAll]);

  useEffect(() => {
    if (!fuel?.rob) return;
    setFuelRobForm({ openingRob: String(fuel.rob.openingRob ?? 0), closingRob: String(fuel.rob.closingRob ?? 0) });
  }, [fuel]);
  ...
  const renderActFields = () => {
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <Select
              value={actForm.berth}
              onChange={(e) => setActForm((p) => ({ ...p, berth: e.target.value, berthSub: "" }))}
            >
          ...
          <div>
            ...
            <Select
              value={actForm.berthSub}
              onChange={set("berthSub")}
              disabled={!actForm.berth}
            >
              <option value="">-- เลือกท่า --</option>
              {(BERTH_SUB_OPTIONS[actForm.berth] ?? []).map((o) => (
                <option key={o.value} value={o.value}>{o.label}</option>
              ))}
            </Select>
          </div>
    ...
    if (isAnch) return (
      <div className="space-y-3">
        {TimeFields}
        <div>
          <label className="text-sm text-slate-600">สถานที่</label>
          <Input value={actForm.anchorLocation} onChange={set("anchorLocation")} placeholder="เช่น อ่าวไทย" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div><label className="text-sm text-slate-600">จำนวนตู้ 20"</label><Input value={actForm.container20Count} onChange={set("container20Count")} placeholder="เช่น 10" /></div>
          <div><label className="text-sm text-slate-600">จำนวนตู้ 40"</label><Input value={actForm.container40Count} onChange={set("container40Count")} placeholder="เช่น 5" /></div>
        ...
        <div>
          ...
          <Input value={actForm.reeferCount} onChange={set("reeferCount")} placeholder="เช่น 3" />
        </div>
        {GeneratorFields}
        {DeckgenFields}
        {DraftFields}
    ...
    if (isFSW) return (
      <div className="space-y-3">
        {TimeFields}
        <div>
          <label className="text-sm text-slate-600">ความเร็วเฉลี่ย (นอต)</label>
          <Input value={actForm.avgSpeed} onChange={set("avgSpeed")} placeholder="เช่น 12.5" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CurrentField}
          <div>
  ...
  const saveRob = async () => {
    ...
    const ok = await alertConfirm({ title: "ยืนยันการบันทึก", text: "ต้องการบันทึก Opening/Closing ROB ใช่หรือไม่", confirmText: "บันทึก", cancelText: "ยกเลิก" });
    if (!ok) return;
    try {
                  ...
                  {tab === "activity" ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Button onClick={openCreateActivity} className="gap-2 cursor-pointer"><Plus size={16} />สร้าง Activity</Button>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button onClick={openCreateBunker} className="gap-2 cursor-pointer"><Plus size={16} />เพิ่มรายการเติม</Button>
                        </div>
                      </div>
                        ...
                        <>
                          <div className="grid gap-3 lg:grid-cols-3">
                            <div className="rounded-xl border border-slate-100 bg-white p-4">
                              ...
                              <Input value={fuelRobForm.openingRob} onChange={(e) => setFuelRobForm((p) => ({ ...p, openingRob: e.target.value }))} placeholder="เช่น 12000" />
                            </div>
                            <div className="rounded-xl border border-slate-100 bg-white p-4">
                              <div className="text-xs text-slate-500 mb-2">น้ำมันคงเหลือตอนจบงาน (ลิตร)</div>
                              <Input value={fuelRobForm.closingRob} onChange={(e) => setFuelRobForm((p) => ({ ...p, closingRob: e.target.value }))} placeholder="เช่น 8600" />
                            </div>
                            <div className="rounded-xl border border-slate-100 bg-white p-4 flex flex-col justify-between">
                              <div>
                                ...
                                <div className="mt-2 space-y-1 text-sm">
                                  ...
                                  <div className="flex justify-between"><span className="text-slate-600">Total Fuel Bunkered</span><span className="font-medium text-slate-900">{fuelBunkeredTotal.toFixed(2)}</span></div>
                                  <div className="flex justify-between"><span className="text-slate-600">Expected Closing</span><span className="font-semibold text-slate-900">{expectedClosing.toFixed(2)}</span></div>
                                  <div className="flex justify-between">
                                    <span className="text-slate-600">Diff</span>
                                    <span className={["font-semibold", Math.abs(diff) <= 0.01 ? "text-emerald-700" : "text-amber-700"].join(" ")}>{diff.toFixed(2)}</span>
                                  </div>
                                </div>
                              </div>
                              <div className="pt-3"><Button onClick={saveRob} className="w-full cursor-pointer">บันทึก ROB</Button></div>
                            </div>
                          </div>
